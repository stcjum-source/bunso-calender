...
export function scheduled(r,ym){const [y,m]=ym.split('-').map(Number);const last=new Date(y,m,0).getDate();const at=n=>`${ym}-${String(n==='last'?last:Math.min(last,+n)).padStart(2,'0')}`;if(r.repeat==='once')return month(r.once)===ym?[r.once]:[];if(r.repeat==='quarterly'&&![1,4,7,10].includes(m))return [];if(r.repeat==='yearly'&&m!==+r.annualMonth)return [];if(r.repeat==='nth'){return r.nths.map(n=>1+(+r.weekday-new Date(y,m-1,1).getDay()+7)%7+(n-1)*7).filter(n=>n<=last).map(at);}if(r.repeat==='weekly'){const first=1+(+r.weekday-new Date(y,m-1,1).getDay()+7)%7;const out=[];for(let day=first;day<=last;day+=7)out.push(at(day));return out;}return [at(r.start)];}
...
export function validState(s){const ds=v=>typeof v==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(v)&&!isNaN(date(v))&&key(date(v))===v;const obj=v=>v&&typeof v==='object'&&!Array.isArray(v);if(!obj(s)||s.version!==1||!Array.isArray(s.rules)||s.rules.length>2000||!obj(s.items)||!obj(s.holidays)||!ds(s.created))return false;let ids=new Set();for(const r of s.rules){if(!obj(r)||typeof r.id!=='string'||ids.has(r.id)||typeof r.title!=='string'||!r.title.trim()||!ds(r.from)||!['monthly','quarterly','yearly','nth','weekly','once'].includes(r.repeat)||!['carry','close'].includes(r.miss)||!['previous','keep','skip'].includes(r.holiday)||typeof r.active!=='boolean'||typeof r.hasDue!=='boolean'||typeof r.note!=='string')return false;ids.add(r.id);if(r.start!=='last'&&!(+r.start>=1&&+r.start<=31))return false;if(!['last','same'].includes(r.deadline)&&!(+r.deadline>=1&&+r.deadline<=31))return false;if(r.repeat==='once'&&!ds(r.once))return false;if(r.repeat==='yearly'&&!(+r.annualMonth>=1&&+r.annualMonth<=12))return false;if(r.repeat==='nth'&&(!Array.isArray(r.nths)||!r.nths.length||!r.nths.every(n=>Number.isInteger(n)&&n>=1&&n<=5)||!(r.weekday>=0&&r.weekday<=6)))return false;if(r.repeat==='weekly'&&!(r.weekday>=0&&r.weekday<=6))return false;}
